...
export function parseAssetId(value: unknown): number {
  const numeric = typeof value === "string" ? Number(value) : value;
  if (typeof numeric !== "number" || !Number.isInteger(numeric) || numeric < 0) {
    throw new InvalidShapeInputError("assetId must be a non-negative integer ASA id.", {
      assetId: value
    });
  }
  return numeric;
...
export interface DorkFiMarketSelector {
  poolAppId: number;
  marketAppId: number;
  assetId: number;
}

export function parseDorkFiMarketSelector(value: Record<string, unknown>): DorkFiMarketSelector {
  if (value.poolAppId === undefined) {
    throw new InvalidShapeInputError("poolAppId is required for Dork.fi lending shapes.");
  }
  if (value.marketAppId === undefined) {
    throw new InvalidShapeInputError("marketAppId is required for Dork.fi lending shapes.");
  }
  if (value.assetId === undefined) {
    throw new InvalidShapeInputError("assetId is required for Dork.fi ASA lending shapes.");
  }

  const assetId = parseAssetId(value.assetId);
  if (assetId === ALGO_ASSET_ID) {
    throw new InvalidShapeInputError(
      "Dork.fi ASA lending shapes do not support native ALGO markets.",
      { assetId }
    );
  }
  ...
  return {
    ...
    marketAppId: parseMarketAppId(value.marketAppId),
    assetId
  };
}
