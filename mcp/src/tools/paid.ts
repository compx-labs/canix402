...
export function registerPaidTools(server: McpServer, client: X402Client): void {
  ...
  server.registerTool(
    ...
    {
      description:
        "Compile one or more unsigned groups for verified execution shapes (POST /execution/quotes). Algorand shapes return Algorand groups; Morpho shapes (base:morpho:vault:*) return unsigned Base calldata. Pass an Algorand userAddress unless the shape is on Base. Omitting a Base address does not change rank, eligibility, price, or access. Pass quotes: [{ shapeKey, input }, ...] (min 1). Required input fields vary by shapeKey — call canix_list_execution_shapes and use each shape's requiredInputs (userAddress is always required). Response data is an ExecutableQuote array in the same order — groups are never merged. Paid: flat ~0.10 USDC via x402 per request on Algorand or Base (not per quote item). On failure, error.details includes quoteIndex and shapeKey. Canix does not sign or submit transactions. Do not guess pool discovery, opt-ins, min-balance, slippage, liquidity limits, or app upgrades — read protocol/docs/execution-shapes/protocol-caveats.md (also GET /execution/shapes meta.caveatsDocsPath) and each shape's docsPath.", // pragma: allowlist secret
      inputSchema: {
        quotes: z
          .array(
            z.object({
